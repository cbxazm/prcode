...
# args = parser.parse_args() 5.25 625
...
def create_batches(data):
    ...
    batches = [data[graph:graph+32] for graph in range(0, len(data), 32)]
    return batches
def test(dataset):
    #model.eval()
    count=0
...
for epoch in epochs:# without batching
    ...
    #test(validdata)
    # torch.save(model, 'models/gmngcj' + str(epoch + 1) + ".pkl")
    devresults=test(validdata)
    # devfile=open('gcjresult/'+args.graphmode+'_dev_epoch_'+str(epoch+1),mode='w')
    devfile=open('gcjresult/'+'astandnext'+'_dev_epoch_'+str(epoch+1),mode='w')
    for res in devresults:
        devfile.write(str(res)+'\n')
    devfile.close()
    # test(testdata)
    testresults=test(testdata)
    resfile=open('gcjresult/'+'astandnext'+'_epoch_'+str(epoch+1),mode='w')
