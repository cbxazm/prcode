...
dataset = [
    [data_folder + '/new_first_msr_pairs.txt', 1, 'train'],
    [data_folder + '/new_second_msr_pairs.txt', 1, 'test'],
    [data_folder + '/new_first_nondup.txt', 0, 'train'],
    [data_folder + '/new_second_nondup.txt', 0, 'test'],
]
...
errorpath = "data/error/getpull.txt"
# ------------------------------------------------------------
    ...
    if code_sim_type == 'tfidf':
        ...
        for pull in pulls:  # only added code
            try:
                ...
                localfile.write_to_file(errorpath, 'Error on get {}'.format(pull['url']))
...
def get_sim(repo, num1, num2):
    ...
    return get_pr_sim_vector(p1, p2, repo)

def get_sim_new(repo, num1, num2):
    p1 = get_pull(repo, num1)
    p2 = get_pull(repo, num2)
    return get_pr_sim_vector_new(p1, p2, repo,num1,num2)

def get_sim_before_new(repo, num1, num2):
    p1 = get_pull(repo, num1)
    p2 = get_pull(repo, num2)
    return get_pr_sim_vector_before_new(p1, p2, repo,num1,num2)

def get_sim_wrap(args):
    return get_sim(*args)

def get_sim_wrap_new(args):
    return get_sim_new(*args)

def get_sim_wrap_before_new(args):
    return get_sim_before_new(*args)
        ...
        for label in [0, 1]:
            ...
            for z in p[r]:
                if z[2] == label:
                    ...
                    # result.append(get_sim_wrap(curtuple))
                    # result.append(get_sim_wrap(curtuple))
            with Pool(processes=5) as pool:
                result = pool.map(get_sim_wrap_before_new, pairs)
            X.extend(result)
...
def beforegmnclassify(model_type=default_model):
